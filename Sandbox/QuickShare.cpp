 ...
 * First, the two application decide what they are, server or client (sender or receiver respectively).
 * Then the user can either specify a file, or a file path.
 * Then the actual transfer occurs.
 *
 * The application breaks the source file into chunks. Then transfered as a HTTP request along with a hash. Once on the receiving end, it computes the hash and checks if the packet contained any
 * errors and if so, sends a resent notification. Once successfully received, it merges all of them together to build the package back together.
 */
...
// allocation to be customized. The allocator_type type and get_allocator()
...
struct CommandArgList {
	CommandArgList() = default;
	CommandArgList(int argc, char* argv[])
	{
